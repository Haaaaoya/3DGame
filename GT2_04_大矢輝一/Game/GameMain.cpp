...
#include "Game/StageCreate/StageCreate.h"
...
void GameMain::Update(const DX::StepTimer& timer)
{
	...
	if (m_pScene != nullptr)
	{
		...
		m_clear = m_pScene->GetClearFlag();
		m_stageNum = m_pScene->GetStageNum();
	}
}
...
void GameMain::CreateScene()
{
	...
	case GAME_SCENE::STAGECREATE:
	{
		m_pScene = new StageCreate();
		break;
	}
	case GAME_SCENE::PLAY:
	{
		m_pScene = new PlayScene(m_stageNum);
		break;
	}
	case GAME_SCENE::RESULT:
	{
		m_pScene = new ResultScene(m_clear);
		break;
	}
	default:
	{
